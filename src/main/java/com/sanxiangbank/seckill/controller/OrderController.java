...
public class OrderController {
     ...
     * 下单接口：乐观锁更新库存 + 令牌桶限流
     * service层：
     * 先查Stock，如果Stock的Sale和Count相同，无库存，抛异常，反之继续
     * 用Stock.sale去尝试更新库存WHERE id = #{id,jdbcType=INTEGER} AND sale = #{sale,jdbcType=INTEGER}
     * update的成功返回是matched的数量，顺带更新库存，反之update失败，因为拿到的是老版本，有人先行一步了
     * 最后createOrder(stock);创建订单
     *
     * @param sid
     * @return
     */
...
//    //校验库存(悲观锁for update)
//    Stock stock = checkStockForUpdate(sid);//for update 加行锁
//    //更新库存
//    saleStock(stock);
//    //创建订单
//    createOrder(stock);
